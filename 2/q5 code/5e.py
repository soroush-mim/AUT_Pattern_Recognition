...
test_mask = cv2.imread('leo2_mask.png',0)
...
for i in range(test_img.shape[0]//8):
    for j in range(test_img.shape[1]//8):
        ...
        if counter_messi[x]*prior_messi>counter_field[x]*prior_field:
            if np.sum(test_mask[8*i:8*(i+1),8*j:8*(j+1)]/255)<33:
                error+=1
        else:
            if np.sum(test_mask[8*i:8*(i+1),8*j:8*(j+1)]/255)>32:
                error+=1
                
error = error / ((test_mask.shape[0]//8) * (test_mask.shape[1]// 8))

print("prob of error: ", error)
            
#prob of error:  0.2912941176470588